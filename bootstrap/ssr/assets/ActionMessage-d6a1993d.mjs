...
const _sfc_main = {
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.html == "") {
        _push(`<div style="${ssrRenderStyle(__props.on ? null : { display: "none" })}" class="text-sm text-gray-600">`);
        ssrRenderSlot(_ctx.$slots, "default", {}, null, _push, _parent);
        _push(`</div>`);
      } else {
        _push(`<div style="${ssrRenderStyle(__props.on ? null : { display: "none" })}" class="text-sm text-gray-600">${__props.html}</div>`);
      }
      _push(`</div>`);
    };
  }
