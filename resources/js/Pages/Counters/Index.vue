...
watch(search, Debounce(function (value) {
    router.get('/counters', { search: value },
               {
                   // preserveState: true,
                   replace: true,
               }
...
<template>
    <AppLayout title="AirPortal JFK Terminal 4">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                AirPortal JFK Terminal 4&nbsp;|&nbsp;Counters
            </h2>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                >
                    <table class="min-w-full divide-y divide-gray-200 border-separate">
                        ...
                        <tbody>
                            <tr v-if="!counters.data || !counters.data.length">
                                <td>
                                    No data to display
                                </td>
                            </tr>
                            <tr
                                v-for="counterData in counters.data"
                                v-else
                                :key="counterData.id"
                            ...
                            >
                                <td
                                    v-if="canEditCounters"
                                    v-tippy="{ content: 'Click to edit counter' }"
                                    class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap border cursor-pointer"
                                    @click="editCounter(counterData.id)"
                                >
                                    {{ counterData.counter_location }}
                                </td>
                                <td
                                    v-else
    ...
    <DialogModal
        :show="confirmingCounterDeletion"
        @close="closeModal"
    >
        <template #title>
        ...
        <template #content>
            <span>Are you sure you want to delete this counter?</span>
        </template>
        <template #footer>
            ...
            <DangerButton @click="deleteCounter">
                YES
            </DangerButton>
        </template>
    </DialogModal>
