...
class Router extends LaravelRouter
{
     ...
     * @var string[]
     */
    public $headers = ['Domain', 'Method', 'URI', 'Name', 'Action', 'Middleware', 'Vendor', 'Group'];
    ...
    public static function toResponse($request, $response)
    {
        ...
        if ($response instanceof PsrResponseInterface) {
            $response = (new SymfonyResponse())->createResponse($response);
    ...
    {
        $excluded = collect($excluded)->map(fn ($name) => (array) MiddlewareNameResolver::resolve($name, $this->middleware, $this->middlewareGroups))->flatten()->values()->all();

        $middleware = collect($middleware)->map(fn ($name) => (array) MiddlewareNameResolver::resolve($name, $this->middleware, $this->middlewareGroups))->flatten()->reject(function ($name) use ($excluded) {
            if (empty($excluded)) {
                return false;
            }
